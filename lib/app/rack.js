...
var path = require('path'),
    ...
    cookie = require('./middlewares/cookie'),
...
exports.initialize = function (app) {
    var rack = app.rack,
        ...
        faviconMaxAge = (devOrTest ? 0 : ONE_WEEK_MS),
    ...
    rack.use(function (context) {
        if (/^\/(css|js)(-[a-zA-Z0-9]+)?(\/|$)/.test(context.url)) {
            var url = context.url.replace(
                /^\/(css|js)(-[a-zA-Z0-9]+)?(\/|$)/,
                function (matched, type, rev, end) {
                    return '/' + type + end;
                }
            );
    ...
    if (!devOrTest) {
        rack.use(compression(), { connect: true });
    }
    ...
    rack.use(cookie(config.cookie.secret));
